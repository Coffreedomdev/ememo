...
CF2211& CF2211::stateChange(char ch){
  // Serial.printf("state change:[%c]\n",ch);
  if (imgbuff[0] == 16) return *this;
  if (ch > 'A' && ch != 'W') ch = '+';
  if (oldstate == ch) return *this;
  oldstate = ch;
...
CF2211& CF2211::quickShow(){
    ...
    if (forceSlowDraw) {
      forceSlowDraw = false;
      epd.DisplayFrame2(0, imgbuff);
    }
    else epd.DisplayFrame2A(imgbuff);
  return *this;
}
...
void CF2211::show(){
    std::lock_guard<std::mutex> lck(epd_mtx);
    epd.DisplayFrame2(random(10), imgbuff);
...
void CF2211::poweroff(bool isBatteryLow){
    clear().log(100,60,&Font48,"Bye");
    if (!isBatteryLow) chkBattery(false);
    else  stateChange('$');
    forceSlowDraw = true;
    quickShow();
    // delay(1000);
...
void CF2211::chkBattery(bool updateScreen){
  int bl = estBatteryV();
  // Serial.printf("power level:%u mV",bl);
  if (updateScreen) {
    if (!digitalRead(charge_pin)) stateChange('*');
    else if (bl>4100) stateChange('(');
    else if (bl>3900) stateChange('\'');
    else if (bl>3700) stateChange('&');
    else if (bl>3450) stateChange('%');
    else if (bl>3300) stateChange('$');
    // else quickLog(10,154,&Font16,"bat:%u mV",bl).show();
    else poweroff(true);
  }
  else {
  // $%&'()*+
    ...
    else if (bl>3450) quickLog(246,154,&Font20,"%s","%");
    else if (bl>3300) quickLog(246,154,&Font20,"%s","$");
    // else  quickLog(10,154,&Font16,"bat:%u mV",bl).show();
    else poweroff(true);
  }
}
...
void CF2211::deepSleep(int sec){ // sec <10 force to sleep!
    ...
    // if (btymV > config->btLevelToSleep) { 
      Serial.printf("[CF2211]Power Level:%u mV,no sleep!,battery Level :%u\n",btymV,config->btLevelToSleep); 
      return; 
    } 
    ...
    epd.Sleep();
    Serial.printf("[CF2211]deep sleep [%u] seconds.\n\n",sec);
    Serial.flush(); 
...
void CF2211::sleep(int sec){
  Serial.printf("[CF2211]sleep:%u\n",sec);
  if (sec <10) sec += 60;
  config->btLevelToSleep = 6000; // for deepsleep test
  if (estBatteryV()>config->btLevelToSleep){
    delay(sec*1000);
  }
  else {
    deepSleep(sec); 
